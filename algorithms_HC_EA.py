...
def calc_HC_tour(tsp): return HC_tour(tsp, 100000)

# //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
# //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
# P2 IMPLEMENTATION, EA
...
def partially_mapped_crossover(parent1, parent2):
    ...
    def map_segment(offspring, parent, other_parent):
        for i in range(start, end + 1):
            if other_parent[i] not in offspring:
                ...
                position = i
                while offspring[position] != -1: position = other_parent.index(parent[position])
    ...
    def fill_rest(offspring, parent):
        for i in range(size): 
            if offspring[i] == -1: offspring[i] = parent[i]
...
def create_child(selected_population, recombination_op, mutation_op):
    ...
    if random.randint(0, 100) < 25: # Change the probability that a mutation is applied to the child (default: 25)
        match mutation_op:
...
def create_offspring(selected_population, population_size):
    ...
    while len(new_population) < population_size:
        new_population.append(create_child(selected_population, 1, 1)) # Change the recombination and mutation operators used (default: 1 and 1)
...
def EA_tour(tsp, population_size, max_generations):
    ...
    for generation in range(max_generations):
        min_k = population_size / 2 # Change the minimum value of k aka the min amount of individuals viewed by the tournament selection (default: 2)
...
def calc_EA_tour(tsp):
    ...
    return EA_tour(tsp, 30, 3500) # Change population size (default: 30) and max generation (default: 3500; max: 100.000)
